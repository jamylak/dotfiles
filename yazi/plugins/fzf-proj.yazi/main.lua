...
local function entry()
	...
	-- Command("fzf"):cwd(cwd):stdin(Command.INHERIT):stdout(Command.PIPED):stderr(Command.INHERIT):spawn()
		Command("sh")
		:arg({ "-c",
			"ls -dt /Applications/ /tmp/ $HOME/ $HOME/.local/share/ $HOME/.local/share/nvim/lazy/ ~/.Trash/ ~/.config/ ~/Desktop/ ~/Downloads/ ~/bar/ ~/bar/*/ ~/proj/ ~/proj/*/ ~/.config/dotfiles* ~/.config/nvim* | fzf --bind ctrl-j:accept" })
		:cwd(cwd)
