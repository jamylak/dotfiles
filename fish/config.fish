...
function __smart_cd_or_insert_path
    ...
    set result (ls -dt /Applications /tmp (eval echo ~) ~/.local/share/ ~/.local/share/nvim/lazy/ ~/.Trash ~/.config ~/bar/ ~/Desktop/ ~/bar/* ~/proj/ ~/proj/* ~/.config/dotfiles ~/.config/nvim | fzf --bind 'ctrl-j:accept')
    if test -z "$result"
        return
    end
