...
abbr -a dotf --position anywhere ~/.config/dotfiles
...
function fish_user_key_bindings
    ...
    bind -M insert ctrl-enter 'commandline -r "tmux_session_fzf"; commandline -f execute'
...
function tmux_fzf
    set dir (fish ~/.config/dotfiles/scripts/fzf_proj_paths.fish | fzf)
    if test -n "$dir"
        builtin cd -- "$dir"
        set name (tmux_dir_session_name "$dir")
        tmux_attach "$name" "$dir"
...
set -gx FZF_DEFAULT_OPTS "--pointer=🔥 --layout=reverse --info=inline --height=80%  --bind=ctrl-h:accept --bind=ctrl-i:accept --bind=ctrl-e:accept --bind=ctrl-j:accept --bind=alt-j:accept --bind=alt-m:accept"
